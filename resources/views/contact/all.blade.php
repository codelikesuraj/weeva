    ...
    <div class="card mb-4">
        <div class="card-header">
            <h3 class="d-flex justify-content-between align-items-center">
                ...
                <div>
                    ...
                    <a class="btn btn-success" href="{{ route('contact.create') }}">Add new</a>
                </div>
            </h3>
        </div>
        <div class="card-body p-0 m-0">
            <div class="table-responsive">
                <table
                    class="text-center table table-bordered border-dark table-striped table-responsive table-sm align-middle">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>Name</th>
                        ...
                        @if ($contacts && $contacts->count() >= 1)
                            @foreach ($contacts as $contact)
                                <tr>
                                    ...
                                    <td>
                                        <a class="text-dark"
                                            href="{{ route('order.issuedBy', [$contact->id]) }}">{{ $contact->name }}</a>
                                    </td>
                                    <td>{{ $contact->phone ? $contact->phone : 'n/a' }}</td>
                                    <td>{{ ucfirst($contact->type) }}</td>
                                    <td>
                                        <a class="py-1 btn btn-outline-danger text-decoration-none"
