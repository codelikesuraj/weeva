<x-app-layout>
    <x-slot name="title">Orders | Edit</x-slot>
    <h1 class="m-4 mx-1">Orders</h1>
    <a href="{{ route('order.show', [$order->id]) }}" class="m-2 btn btn-outline-danger">Go back to order</a>
                ...
                <div class="row">
                    ...
                    <div class="mb-3 col-lg-3 col-8">
                        ...
                        <input id="date_issued" class="form-control" type="date" name="date_issued"
                            value="{{ $order->date_issued }}" required />
                    </div>
                    <!-- Deadline -->
                    <div class="mb-3 col-lg-3 col-8">
                ...
                <div class="mb-3 row">
                    <div class="col-12 col-lg-6">
                        ...
                        <textarea id="description" class="form-control" type="text" rows="4" name="description"
                            placeholder="Enter order description here" required>{{ $order->description }}</textarea>
                    </div>
                </div>
                        ...
                        <select id="issued_by" class="form-select" name="issued_by" required>
                            ...
                            @foreach ($contacts as $contact)
                                <option value="{{ $contact->id }}"
                                    {{ $order->issued_by == $contact->id ? 'selected' : '' }}>
